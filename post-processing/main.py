...
# GCS Cache Configuration - Updated to poc_binod
...
def load_global_data():
    ...
    # 2. Slow Path: BigQuery (Runs once to build cache)
    logger.info("[cache-refresh] Building Cache from BigQuery (this takes ~11s)...")
    t_bq_start = time.perf_counter()
    try:
        # Load Phases from newsradar (source)
        ...
        # Save to GCS Cache in poc_binod
        cache_data = {
        ...
        logger.info(f"[cache-save] SUCCESS: gs://{CACHE_BUCKET}/{GEOMETRY_CACHE_PATH} created. BQ Time: {time.perf_counter() - t_bq_start:.2f}s")
...
async def pipeline():
    ...
    try:
        ...
        logger.info(f"[json-download] '{name}' fetched in {time.perf_counter()-t_json_start:.3f}s")
        ...
        logger.info(f"[item-processing] Valid Rows: P:{len(v_p)}, E:{len(v_e)}, A:{len(v_a)} in {time.perf_counter()-t_proc_start:.3f}s")

        t_bq_start = time.perf_counter()
        def bq_in(table, rows):
            if not rows: return []
            err = bq_client.insert_rows_json(table, rows)
            if err: logger.error(f"[bq-error] {table}: {err}")
            return err

        loop = asyncio.get_event_loop()
        await asyncio.gather(
            loop.run_in_executor(None, bq_in, TABLE_PROJECTS, v_p),
            loop.run_in_executor(None, bq_in, TABLE_EVENTS, v_e),
            loop.run_in_executor(None, bq_in, TABLE_AREAS, v_a)
        )
        logger.info(f"[bq-insert] Streamed to BigQuery in {time.perf_counter()-t_bq_start:.3f}s")
